...
export class AppComponent {
  ...
  photos_visible = this.photos;

  category = [
    {idcat: 1, cat: "Kwiaty", checked: true},
    {idcat: 2, cat: "Zwierzęta", checked: true},
    {idcat: 3, cat: "Samochody", checked: true}
  ];

  filtrujKategorie(category: number){
    let wynik = this.photos.filter(photos => {
      const category = this.category.find(category => category.idcat == photos.category)
  ...
  filtruj2(){
    ...
    this.photos.forEach(photo => {
      this.category.forEach(cat => {
        if(photo.category == cat.idcat && cat.checked){
          filtered.push(photo);
        }
      });
    });
    this.photos_visible = filtered;
  }
