...
export class ListFormComponent implements OnInit {
  listForm: FormGroup;
  editMode = false;
  listId: string = ''; // Inicjalizacja pustym stringiem
  newItemName = '';
  ...
  // Metody pomocnicze dla template'a - pozwalają uniknąć błędów null
  getName() {
    return this.listForm.get('name');
  }

  getStore() {
    return this.listForm.get('store');
  }

  getDate() {
    return this.listForm.get('date');
  }

  getItemName(index: number) {
    return this.items.at(index).get('name')?.value;
  }

  getItemPurchased(index: number) {
    return this.items.at(index).get('purchased')?.value || false;
  }
  ...
  togglePurchased(index: number): void {
    const item = this.items.at(index).get('purchased');
    if (item) {
